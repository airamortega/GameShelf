      ...
      <div v-if="modelValue" class="fixed inset-0 z-[200] flex items-center justify-center p-6">
        ...
        <div class="relative w-full max-w-sm bg-slate-900 border border-white/10 rounded-[40px] p-8 shadow-2xl overflow-hidden">
          ...
          <header class="text-center mb-8">
            ...
            <h3 class="text-2xl font-black text-white uppercase italic tracking-tighter">Editar Lista</h3>
            <p class="text-gray-500 text-sm">Cambia el nombre o la privacidad</p>
          </header>
...
<script setup>
import { Pencil, Globe, Lock, Loader2 } from 'lucide-vue-next'

const props = defineProps({
  modelValue: Boolean,
  list: Object // Recibimos la lista a editar
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    emit('updated') // Avisamos al padre para que refresque
    close()
  } catch (err) {
